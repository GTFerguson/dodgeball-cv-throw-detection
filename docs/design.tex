...
last throw at that side. Blocks move no count and are not claimed. Report \S2.1.}
...
used only at the set start, where the ball line-up gives it a window. Report \S2.2.}
...
blind second labelling pass. Each of these has its measurement and its reasoning in the
report (\texttt{docs/report.pdf}, \S2), and the shipped design of each stage is in \texttt{docs/architecture/}.
