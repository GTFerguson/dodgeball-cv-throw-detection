...
| [rebound.md](rebound.md) | What the ball did at the player it reached: SAM2 seeded on the chain's last blob and followed through the contact; the turn tells a hit from a miss, and breaks the recency tie |
| [outcome.md](outcome.md) | Hit, catch or miss from persistent steps in a side's in-play count, attributed to the last throw at that side or to the ball seen to turn — and why the whistle and single tracks could not say |
