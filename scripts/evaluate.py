...
With no predictions file the candidate proposals are scored as a timeline that
claims only "a throwing motion here" - the baseline row for every stage that
follows. See ``src/evaluate.py`` for what is scored and how it matches.
...
def main() -> int:
    ...
    ap.add_argument("--predictions", type=Path, help="timeline JSON; default: the candidates")
